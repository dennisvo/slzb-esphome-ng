...
4. **Chip and role mismatch** are detected at boot by cross-referencing the wire-probed chip family / role against the compile-time `radio*_chip` / `radio*_role` substitutions. Mismatches emit `ESP_LOGE` in the boot log (log-only, not exposed as HA entities). The wire evidence itself is published as `radio*_chip_probed` and `radio*_role_probed` sensors — HA compares those against the declared `radio*_chip` / `radio*_role` sensors to draw its own verdict for the update entity's availability template. See [radio-probe-reference.md §6i](radio-probe-reference.md#6i-wire-protocol-vs-firmware-role--a-taxonomy-fix) for the detection rules per wire protocol.
