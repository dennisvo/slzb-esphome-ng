...
- **Live installed-version probe** — ESPHome speaks ZNP `SYS_VERSION` to the radio at boot, publishes the real version as a text_sensor. In-memory only, no cached fallback. See firmware-updates.md §6 for the frame layout.
...
| `type` | `radio*_role` (hw_defs) | `coord`→`"0"`, `router`→`"1"`, `rcp`/`ncp`→`"2"`. See [firmware-updates.md](firmware-updates.md) §6g permutation matrix |
...
- Points to `docs/development-considerations.md` §5 and §6 for the full story
...
| `components/radio_probe/` | Custom component: per-radio boot-time probe dispatched on `radio*_protocol`. See [ROADMAP.md](ROADMAP.md) Steps 1–3 for phased build (ZNP live → Spinel live → EZSP/Z-Wave stubs) | Medium (custom C++/Python) | UART bus (`setup_priority::BUS`, ~1000); runs at `on_boot: priority: 250` before `serial_proxy` attaches |
...
- **Two-dim role encoding** — `radio*_protocol` + `radio*_role` (see [firmware-updates.md](firmware-updates.md) §6g). Single-dim `radio*_firmware_type` is retired.
...
Cross-check against the 25 sections catalogued in [slzb-os-menu-analysis.md](research/slzb-os-menu-analysis.md). Groups pages by whether their function is portable to our ESPHome + HA model.
