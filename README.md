...
- **Radio firmware version reported to Home Assistant.** Each radio publishes its installed firmware version as a diagnostic sensor. **CC26xx ZNP (Zigbee) and EFR32 Spinel (Thread) probes are live** — they return the actual firmware version straight from the radio at boot. **EZSP (EFR32 Zigbee) and Z-Wave sensors ship as stubs** that publish `"unknown (<protocol> probe not implemented in v1)"` until the real probes land in a later v1.x point release. A small HA template snippet reads SMLIGHT's public firmware catalog and shows an **"update available"** entity per radio, filtered by the `prod` / `dev` channel declared in your device YAML. Read-only; radio flashing itself is not part of v1.
